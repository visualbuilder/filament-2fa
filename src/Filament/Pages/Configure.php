...
class Configure extends SimplePage
{
    ...
    public function form(Schema $schema): Schema
    {
        ...
        return $schema->components([
            TextInput::make('two_factor_code')
                ...
                ->maxLength($digits)
                ->autofocus()
                ->helperText(__('Generated by your authenticator app.'))
                ->autocomplete(false)
                ->live()
    ...
    public function content(Schema $schema): Schema
    {
        return $schema->components([
            ...
            Section::make(__('Scan this QR code with your authenticator app'))
                ->schema([
                    Grid::make()->schema([
                ...
                ->schema([
                    ...
                    Section::make()
                        ->schema([
                            ...
                            UnorderedList::make(fn() => collect($this->recoveryCodes)
                                ->map(function($item) {
                                    $code = $item['code'] ?? $item;
                                    $usedAt = $item['used_at'] ?? null;

                                    $text = Text::make($code)->fontFamily(\Filament\Support\Enums\FontFamily::Mono);

                                    // Apply strikethrough if the code has been used
                                    if ($usedAt) {
                                        $text->extraAttributes(['class' => 'line-through opacity-50']);
                                    }
    ...
    protected function getConfirmFormAction(): Action
    {
        return Action::make('confirm')
            ->label(__('Confirm'))
            ->submit('confirmTwoFactorFromForm');
    }
    ...
    {
        // Prevent processing if already enabled (prevents duplicate notifications)
